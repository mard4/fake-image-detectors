...
class DataloaderTRUEFAKE:
    ...
    def load_data(self, real_or_fake, batch_size=32, transform=None, max_images_per_class=None, ALL_MODELS='YES', models=None):
        """
        ...
        if real_or_fake == 'Fake':
            ...
            if ALL_MODELS.upper() == 'YES':
                models_to_load = [model for model in os.listdir(self.fake_base_path) if os.path.isdir(os.path.join(self.fake_base_path, model))]
                print("ALL_MODELS is set to 'YES'. Loading all available models.")
            elif ALL_MODELS.upper() == 'NO':
                if not models:
                    raise ValueError("When ALL_MODELS is 'NO', the 'models' parameter must be provided as a list of model names.")
                models_to_load = models
                print(f"ALL_MODELS is set to 'NO'. Loading specified models: {models_to_load}")
            else:
                raise ValueError("ALL_MODELS parameter must be either 'YES' or 'NO'.")
...
if __name__ == "__main__":
    ...
    ### LOADER FOR TRUEFAKE (BUT WE DONT HAVE THE LABELS REGARDING THE REAL IMAGE)
    loader_TRUEFAKE = DataloaderTRUEFAKE()
    ...
    dataloader_TRUEFAKE_fake = loader_TRUEFAKE.load_data(
        ...
        ALL_MODELS='YES'  # Load all available models
    )
    ...
    dataloader_TRUEFAKE_fake = loader_TRUEFAKE.load_data(
        ...
        batch_size=64,
        max_images_per_class=100,  # Limit per category
        ALL_MODELS='NO',  # Do not load all models
